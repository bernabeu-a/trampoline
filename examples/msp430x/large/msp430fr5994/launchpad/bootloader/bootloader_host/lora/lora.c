...
void readBurstRegister(uint8_t reg, uint8_t* dest, uint8_t len){
	...
	while(len--) {
		while (!(UCA3IFG & UCTXIFG));           // USCI_B0 TX buffer ready?
		UCA3TXBUF = 0xFF;                  // Send value over SPI to Slave
		while (!(UCA3IFG & UCRXIFG));           // USCI_B0 RX Received?
        *dest++ = UCA3RXBUF;
...
uint8_t recv(uint8_t* buf, uint8_t* len){
    ...
    while((readRegister(RH_RF95_REG_12_IRQ_FLAGS) != RH_RF95_RX_DONE));
    ...
    // Clear Rx flag
	writeRegister(RH_RF95_REG_12_IRQ_FLAGS, RH_RF95_RX_DONE);
