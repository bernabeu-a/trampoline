...
/* State Machine of Concerto */
static enum {RUNNING, CHARGED, WAITINGu, WAITINGl, OFF} state_concerto = OFF;
...
int main_boot( void ) {
    ...
    while(1){
        switch (state_concerto){
            /* Only transition possible is deploy */
            case OFF:
                    state_concerto = WAITINGl;
                    break;
            /* Waiting complete loading (reception application) */
            case WAITINGl:
                    while(1){
                        uint16_t ii;
                        uint8_t rx_length = TX_BUF_SIZE;
                        ...
                        /* All reception OK, state is now Charged */
                        if (ret == RET_JUMP_TO_APP){
                            state_concerto = CHARGED;
                            break;
                        }
                    }
                    break;
            /* Waiting update (reception new application) */
            case WAITINGu:
                    while(1){
                            uint16_t ii;
                            uint8_t rx_length = TX_BUF_SIZE;
                    ...
                    break;
            /* All application data received, check CRC and push to app memory */
            case CHARGED:
                    TI_MSPBoot_AppMgr_JumpToApp();
                    state_concerto = RUNNING;
                    break;
            /* Application ready, start it */
            case RUNNING:
