...
VAR(dtype, AUTOMATIC) PADDING_BUFFER[PADDING_BUFFER_LENGTH] = {0};

VAR(dtype, AUTOMATIC) FILTER_BUFFER[FILTER_BUFFER_LENGTH] = {0};

matrix *globalAveragePooling(matrix *result, matrix *input, uint16_t num_filter){
    /*
     * Implementation of global average pooling
     */
    uint16_t filter_length, i, j, input_offset;
    filter_length = input->numCols * input->numRows;
    
    for(i = 0; i< num_filter; i++){
        int32_t sum = 0;
        for(j = 0; j < filter_length; j++){
            input_offset = i * filter_length;
            sum += input->data[input_offset];
        }
        result->data[i] = fp_div((int16_t) sum, num_filter, FIXED_POINT_PRECISION);
    }
    return result;
}
...
matrix *filter_simple(matrix *result, matrix *input, matrix *filter, uint16_t precision, uint16_t stride_numRows, uint16_t stride_numCols){
    ...
    for (i = 0; i <= input_numRows - filter_numRows; i += stride_numRows){
        for (j = 0; j <= input_numCols - filter_numCols; j += stride_numCols){
            ...
            result->data[result_offset] = result->data[result_offset] + sum;  // add bias
            result_offset++;
        }
    }
    return result;
...
//        }
        ...
        #else
        filter_simple(result, input, filter, precision, stride_numRows, stride_numCols);
        #endif
